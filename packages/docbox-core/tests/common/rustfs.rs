...
const TEST_ACCESS_KEY: &str = "rustfsadmin";
const TEST_ACCESS_SECRET: &str = "rustfsadmin";
...
#[allow(dead_code)]
pub async fn test_tenant_storage(tenant: &Tenant) -> (StorageLayer, ContainerAsync<RustFS>) {
    let storage_container = test_rustfs_container().await;
